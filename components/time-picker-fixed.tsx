...
export function TimePickerFixed({ value, onChange, className }: TimePickerProps) {
  const [open, setOpen] = React.useState(false)

  const selectedHour = value ? parseInt(value.split(":")[0]) : null

  const updateTime = (hour: number) => {
    onChange(`${pad(hour)}:00`) // minutes always 00
    setOpen(false)
  }
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          {value || <p className="text-[14px] font-normal">Pick a time</p>}
          <Clock className="ml-auto h-4 w-4 opacity-50" />
        </Button>
      </PopoverTrigger>

      <PopoverContent className="w-fit p-2 bg-[#1A1A1A] border-[#3A3A3A]" align="start">
        <ScrollArea className="h-32 w-14">
